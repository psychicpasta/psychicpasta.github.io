...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	//..This callback is used for scrolling evenrts
...
void ViewManager::Mouse_Scroll_Callback(GLFWwindow* window, double xScrollPos, double yScrollPos)
{
	//..This will change the speed of the camera
	//..Scrolling up causes slowness while scrolling down causes it to gain speed
	g_pCamera->ProcessMouseScroll(yScrollPos);
}
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	//..Processes camera moving up
	if (glfwGetKey(m_pWindow, GLFW_KEY_Q) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(UP, gDeltaTime);
	}

	//..Process camera view moving down
	if (glfwGetKey(m_pWindow, GLFW_KEY_E) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(DOWN, gDeltaTime);
	}

	//..Switches the view projection to the perspective of the user
	...
	//..Switches the view projection to orthographic
	...
	//..Allows the user to turn off the scene lights
	if (glfwGetKey(m_pWindow, GLFW_KEY_N) == GLFW_PRESS)
	{
		//..Deactivates Light Source 1
		...
		m_pShaderManager->setFloatValue("lightSources[1].focalStrength", 0.0f);
		m_pShaderManager->setFloatValue("lightSources[1].specularIntensity", 0.0f);

		//..Deactivates Light Source 3
		...
		m_pShaderManager->setFloatValue("lightSources[2].focalStrength", 0.0f);
		m_pShaderManager->setFloatValue("lightSources[2].specularIntensity", 0.0f);
	}

	//..Allows the user to restore the scene lights
	if (glfwGetKey(m_pWindow, GLFW_KEY_M) == GLFW_PRESS)
	{
		//..Reapplys Light Source 1
		m_pShaderManager->setVec3Value("lightSources[0].position", 3.0f, 5.0f, 0.6f);
		...
		m_pShaderManager->setVec3Value("lightSources[1].specularColor", 5.0f, 5.0f, 5.0f);
		...
		//..Reapplys Light Source 3
		m_pShaderManager->setVec3Value("lightSources[2].position", -10.0f, 5.0f, 6.0f);
